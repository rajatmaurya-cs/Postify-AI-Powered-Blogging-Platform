...
const BlogList = () => {
  ...
  return (
    <div className="p-4 sm:p-8 animate-in fade-in duration-500 h-full flex flex-col">
      <div className="mb-8">
        <h1 className="text-3xl font-black tracking-tight text-gray-900 mb-2">Publications Space</h1>
        <p className="text-gray-500 font-medium tracking-wide">Manage, moderate, and publish content.</p>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-gray-100/60 overflow-hidden relative flex-1 flex flex-col">
        ...
        <div className="px-8 py-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
           <div className="text-sm font-semibold text-gray-500 flex items-center gap-2">
             ...
             {isFetching && !isFetchingNextPage ? "Syncing publications..." : "All Data Synced"}
           </div>
        </div>
        ...
        <div className="overflow-x-auto flex-1 relative">
          ...
          <table className="w-full text-left border-collapse min-w-[900px] table-fixed">
            <thead>
              <tr className="bg-white border-b border-gray-100/80 text-xs font-bold uppercase tracking-widest text-gray-400">
                <th className="px-8 py-5 w-20">#</th>
                <th className="px-8 py-5 w-[350px]">Title</th>
                <th className="px-8 py-5 w-36">Created</th>
                <th className="px-8 py-5 w-36">State</th>
                <th className="px-8 py-5 w-40">Visibility</th>
                <th className="px-8 py-5 w-24 text-center">Trash</th>
                <th className="px-8 py-5 w-40">Moderator</th>
              </tr>
            </thead>
              ...
              {blogs.map((blog, index) => (
                <tr key={blog._id} className="group hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-8 py-6">
                    <p className="text-base font-bold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-2 pr-4">{blog.title}</p>
                    <p className="text-xs font-medium text-gray-500 mt-1 uppercase tracking-wide bg-gray-100 inline-block px-2 py-0.5 rounded-md">ID: {blog._id.slice(-6)}</p>
                  </td>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-xs shadow-sm shadow-indigo-100">
                        {blog.moderatedBy?.fullName ? blog.moderatedBy.fullName.charAt(0) : "S"}
                      </div>
          ...
          {blogs.length === 0 && !isLoading && !isError && (
            <div className="flex flex-col items-center justify-center py-24 text-center px-4 border-t border-gray-50">
              ...
              <p className="text-gray-500 font-medium">Try adjusting filters or submit a new story.</p>
            </div>
          )}
        </div>
        ...
        <div className="px-8 py-6 flex justify-center border-t border-gray-100 bg-white mt-auto">
          {hasNextPage ? (
            ...
            <p className="text-sm font-bold tracking-widest text-gray-400 uppercase">End of Directory</p>
          )}
        </div>
      </div>
    </div>

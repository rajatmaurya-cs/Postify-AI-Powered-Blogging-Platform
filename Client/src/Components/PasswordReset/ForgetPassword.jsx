...
const ForgetPassword = () => {
  ...
  const { verifyOtp, isVerifying, isVerified, setIsVerified } = useVerifyOtp("RESET_PASSWORD");
  ...
  const onSendOtp = () => {
    ...
    // OR if check endpoint is not needed, simply do: sendOtp(cleaned)
  };
      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur rounded-2xl shadow-2xl p-8">
        <div className="w-full flex flex-col gap-6">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">Verify email with OTP, then set a new password.</p>
          </div>
            ...
            <div className="flex gap-3">
              ...
              {!isVerified && email !== "" && (
                <button
                  ...
                  onClick={onSendOtp}
                >
                  {(sending || checkEmailMutation.isPending) ? "Sending..." : "Send OTP"}
                </button>
              )}
            </div>
          ...
          {otpSent && (
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 flex flex-col items-center gap-3">
              <p className="text-sm font-medium text-gray-700">Enter the 6-digit OTP sent to your email</p>
              ...
              <OtpInput
                ...
                containerStyle={{ display: "flex", justifyContent: "center", gap: "10px", width: "100%" }}
                inputStyle={{
                  width: "44px",
                  height: "46px",
                  borderRadius: "10px",
              ...
              <p className="text-xs text-gray-500">{isVerifying ? "Verifying OTP..." : "Check spam if you don’t see it."}</p>
            </div>
          )}
        </div>
      </div>
